...
class StaticBanner extends Component {
  ...
  componentDidMount = () => {
    ...
    let url =
      "https://qas16.bigbasket.com/content-svc/static-banner/get/banners-list?page=1";
    fetch(url, {
      method: "GET",
      headers: {
        "x-project": "mm-canary",
      ...
      .then((response) => {
        ...
        for (var i = 0; i < response.length; i++) {
          response[i]["bannerType"] = (
            <a href={grootHost + "/bannerdetails/" + response[i]["id"]}>
              {response[i]["bannerType"]}
            </a>
          );
          ...
          response[i]["createdDate"] = new Date(
            response[i]["createdDate"]
          ).toLocaleString();
          response[i]["updatedDate"] = new Date(
            response[i]["updatedDate"]
          ).toLocaleString();
        }
  ...
  handleFilterApi = (columnName, columnValue) => {
    ...
    console.log(filters);
    const url =
      'https://qas16.bigbasket.com/content-svc/static-banner/filter?filters=[{"' +
      k +
      '": "' +
      v +
      '" }]';
  ...
  handleSearchApi = (searchValue) => {
    const s = searchValue;
    const url =
      'https://qas16.bigbasket.com/content-svc/static-banner/search?searchQuery="' +
      s +
      '"';
    console.log(url);
    fetch(url, {
